...
if(empty($step) || $step == 1) {
	...
	<div class="stepstat">
		<ul>
			...
			<li class="unactivated">安装数据库</li>
			<li class="unactivated">设置后台管理员</li>
			<li class="unactivated last">安装完成</li>
		</ul>
...
END;
	if(!$checkok) {
		echo "<div class=\"btnbox marginbot\"><form method=\"post\" action=\"install.php?step=2\"><input type=\"submit\" value=\"强制继续\"><input type=\"button\" value=\"关闭\" onclick=\"windowclose();\"></form></div>";
	} else {
		print <<<END
...
END;
	...
	<form name="themysql" method="post" action="install.php?step=3">
		...
		<table class="tb2">
			<tr>
				<th class="tbopt" align="left">&nbsp;数据库表前缀:</th>
...
END;
	...
	<div class="stepstat">
		<ul>
			<li class="activated">检查安装环境</li>
			<li class="activated">创建数据库</li>
			<li class="current">安装数据库</li>
			<li class="unactivated">设置后台管理员</li>
			<li class="unactivated last">安装完成</li>
		</ul>
...
END;
	...
	<div class="stepstat">
		<ul>
			<li class="activated">检查安装环境</li>
			<li class="activated">创建数据库</li>
			<li class="activated">安装数据库</li>
			<li class="current">设置后台管理员</li>
			<li class="unactivated last">安装完成</li>
		</ul>
		<div class="stepstatbg stepstat5"></div>
	</div>
	</div>
	<div class="main">
	<form name="theuser" method="post" action="install.php?step=6">
...
END;
	...
	$install_date = substr(IN_BUILD, 0, -4).'-'.substr(IN_BUILD, -4, 2).'-'.substr(IN_BUILD, -2, 2).' 00:00:00';

	$t_name="默认模板";
	$t_dir="default";
	$t_html="html";
	$strs=file_get_contents("template/".$t_dir."/install.xml");
	$strs=preg_replace("/type=\"(.*?)\"/", "type=\"".IN_CHARSET."\"", $strs);
	$strs=preg_replace("/name=\"(.*?)\"/", "name=\"".$t_name."\"", $strs);
	$strs=preg_replace("/dir=\"(.*?)\"/", "dir=\"".$t_dir."\"", $strs);
	$strs=preg_replace("/html=\"(.*?)\"/", "html=\"".$t_html."\"", $strs);
	$strs=preg_replace("/CDATA\[(.*?)\]/", "CDATA[".IN_VERSION."]", $strs);
	$ifiles=new iFile('template/'.$t_dir.'/install.xml', 'w');
	$ifiles->WriteFile(convert_xmlcharset($strs, 1), 3);
	...
	$sql_template1="insert into `".tname('template')."` (in_name,in_path,in_default,in_addtime) values ('".$t_name."','template/".$t_dir."/".$t_html."/','1','".$install_date."')";
	...
	$sql_plugin1="insert into `".tname('plugin')."` (in_name,in_dir,in_file,in_isindex,in_type,in_author,in_address,in_addtime) values ('".$p_name."','".$p_dir."','".$p_file."','1','".$p_type."','".$p_author."','".$p_address."','".$install_date."')";
	$sql_plugin2="insert into `".tname('plugin')."` (in_name,in_dir,in_file,in_isindex,in_type,in_author,in_address,in_addtime) values ('阿里云存储','oss','uplog','0','3','missra','http://www.missra.com/','".$install_date."')";
	if(install_db_query($sql_admin, $db) && install_db_query($sql_user, $db) && install_db_query($sql_template1, $db) && install_db_query($sql_template2, $db) && install_db_query($sql_template3, $db) && install_db_query($sql_plugin1, $db) && install_db_query($sql_plugin2, $db)) {
		fwrite(fopen('data/install.lock', 'wb+'), date('Y-m-d H:i:s'));
		show_msg('<p>恭喜！MixMusic 顺利安装完成！</p><p>为了保证数据安全，请手动删除{static/install}目录！</p><p>您的后台管理员帐号与前台会员帐号已经成功建立。</p><p><a href="index.php">进入网站首页</a> 或 <a href="admin.php">进入管理后台</a></p>', 999);
	} else {
...
function show_footer() {
	global $year;
	print<<<END
				<div class="footer">&copy;2015 - $year <a href="http://www.missra.com/" target="_blank">MixMusic</a> Inc.</div>
			</div>
		</div>
	</body>
	</html>
...
function show_msg($message, $next=0, $jump=0) {
	...
	if(empty($next)) {
		...
		if(empty($jump)) {
			...
			$nextstr .= "<span>请稍等...</span><script type=\"text/javascript\">setTimeout(\"location.href='$url_forward';\", 1000);</script>";
		}
	}
	show_header();
	print<<<END
