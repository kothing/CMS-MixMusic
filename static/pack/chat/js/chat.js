...
var f_getURL = function () {
    return "source/pack/upload/save.php";
};
...
var listenMsg = {
    ...
    mid: 0,
    gid: 0,
    queryUrl: in_ssl + "://" + in_server + "/source/pack/api/chat/",
    stop: function () {
    ...
    login: function () {
        $(".chat03_content").html('<img src="static/admincp/images/loading.gif">');
        if (isIE()) {
            $("#send_tips").text("工具->Internet 选项->安全->自定义级别->其他->通过域访问数据源->启用");
        }
        ajax({
            type: "get",
            url: listenMsg.queryUrl + "login.php" + in_info,
            dataType: "jsonp",
            success: function (data) {
                if (data["uid"] == -1) {
    ...
    lock: function (_id) {
        ajax({
            type: "get",
            url: listenMsg.queryUrl + "lib.php?ac=lock&id=" + _id,
            dataType: "jsonp",
            success: function (data) {
                if (data["lock"] == -1) {
    ...
    first: function (_pswd) {
        ...
        ajax({
            type: "get",
            url: listenMsg.queryUrl + "lib.php?ac=first&pswd=" + escape(_pswd),
            dataType: "jsonp",
            success: function (data) {
                if (data["first"] == -1) {
                    layer.msg("请先登录应用中心！", 3, 11);
    ...
    verify: function (_pswd) {
        ajax({
            type: "get",
            url: listenMsg.queryUrl + "lib.php?ac=verify&pswd=" + escape(_pswd),
            dataType: "jsonp",
            success: function (data) {
                if (data["verify"] == -1) {
                    layer.msg("请先登录应用中心！", 3, 11);
    ...
    lib = {
        ...
        reset: function () {
            $.layer({
                ...
                dialog: {
                    ...
                    btn: ["确认", "取消"],
                    yes: function () {
                        window.open(in_ssl + "://" + in_server + "/index.php/setting/resetpw/");
                    },
                    no: function () {
                        layer.msg("已取消重置", 1, 0);
                    }
        ...
        avatar: function () {
            $.layer({
                ...
                area: ["auto", "auto"],
                page: {
                    html: '<embed src="' + in_ssl + "://" + in_server + "/static/pack/upload/camera.swf?ucapi=" + in_avatar + "&input=uid%3D" + $(".talkTo").attr("verify") + '" width="459" height="266" wmode="transparent" allowScriptAccess="always" type="application/x-shockwave-flash"></embed>'
                }
            });
        },
        poker: function () {
            $.layer({
                ...
                area: ["826px", "549px"],
                iframe: {
                    src: in_ssl + "://" + in_server + "/source/pack/api/poker/game.php",
                    scrolling: "no"
                }
            });
        },
        record: function () {
            $.layer({
                ...
                area: ["322px", "211px"],
                page: {
                    html: '<link href="static/pack/chat/live/live.css" rel="stylesheet" type="text/css" /><script type="text/javascript" src="static/pack/chat/live/recorder.js"></script><script type="text/javascript" src="static/pack/chat/live/basic.js"></script><script type="text/javascript" src="static/pack/chat/live/live.js"></script><div class="box"><div style="width:0;height:0;position:fixed"><span id="flashcontent"></span></div><div class="boxt"><span class="fr minitxt">语音直播</span></div><div class="boxm"><div class="boxml"><a class="btn pause" id="live_start"></a> <a class="btn stop" id="live_stop"></a></div><div class="boxmr"><div class="taketime">00:00:00</div><span>时</span><span>分</span><span>秒</span></div></div><div id="live_progress"></div><div class="boxb"><span id="live_tips"></span></div><form id="LiveForm" action="source/pack/chat/savelive.php"><input name="live_file" type="hidden"></form></div>'
                }
            });
        },
        liveing: function (_state, _id, _src) {
            if (_state == 1) {
                $("#" + _id).attr("src", _src.replace(/\?\d+$/, "?") + new Date().getTime());
            } else if (_state == 2) {
                $("#" + _id).attr("src", _src.replace(/\?\d+$/, "?") + new Date().getTime());
                $("#" + _id).attr("autoplay", "autoplay");
                $("#" + _id).attr("onended", "lib.liveing(1, this.id, this.src)");
            } else {
                if (navigator.appName == "Netscape") {
                    lib.liveing(1, _id, _src);
                }
            }
        },
