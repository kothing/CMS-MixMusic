...
if(IN_UPOPEN==1){
	...
	$file=$db->getone("select in_file from ".tname('plugin')." where in_type=3 and in_dir='".IN_REMOTEPK."'");
        $script="../../plugin/".IN_REMOTEPK."/".$file.".php";
        $data="'type':'".$_GET['type']."'";
        if(!is_file($script)){
                $script="uplog.php";
                $data="'type':'".$_GET['type']."','uid':'".$uid."','uname':'".$uname."'";
...
switch($_GET['type']){
	case 'music_audio':
		$ext=IN_UPOPEN==1 || IN_REMOTE==2 ? IN_UPMUSICEXT : "*.*";
		$size=IN_UPOPEN==0 && IN_REMOTE==1 ? @ini_get('file_uploads') ? intval(ini_get('upload_max_filesize')) : 200 : IN_UPMUSICSIZE;
		$desc=IN_UPOPEN==1 || IN_REMOTE==2 ? "请选择".IN_UPMUSICEXT."文件" : "All Files";
		break;
	case 'music_lyric':
		$ext="*.lrc";
		$size=1;
	...
	case 'singer_cover':
		...
		break;
	case 'video_play':
		$ext=IN_UPOPEN==1 || IN_REMOTE==2 ? IN_UPVIDEOEXT : "*.*";
		$size=IN_UPOPEN==0 && IN_REMOTE==1 ? @ini_get('file_uploads') ? intval(ini_get('upload_max_filesize')) : 200 : IN_UPVIDEOSIZE;
		$desc=IN_UPOPEN==1 || IN_REMOTE==2 ? "请选择".IN_UPVIDEOEXT."文件" : "All Files";
		break;
	case 'video_cover':
		$ext="*.jpg;*.jpeg;*.gif;*.png";
		$size=2;
