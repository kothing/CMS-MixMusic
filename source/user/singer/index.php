...
$pos = $posarr[0];
$get = true;
$Arr = getuserpage("select * from ".tname('singer')." where UPPER(substring(".convert_using('in_name').",1,1))='".$letter_arr[$pos]."' and in_uid=".$missra_in_userid." and in_passed=0 or ord(substring(".convert_using('in_name').",1,1))-65536>=".$letter_arr1[$pos]." and  ord(substring(".convert_using('in_name').",1,1))-65536<=".$letter_arr2[$pos]." and in_uid=".$missra_in_userid." and in_passed=0", 10, 4);
}elseif(IsNum($cid)){
$get = $db->getone("select in_id from ".tname('singer_class')." where in_id=".$cid);
$Arr = getuserpage("select * from ".tname('singer')." where in_uid=".$missra_in_userid." and in_passed=0 and in_classid=".$cid." order by in_addtime desc", 10, 4);
}else{
$get = true;
$Arr = getuserpage("select * from ".tname('singer')." where in_uid=".$missra_in_userid." and in_passed=0 order by in_addtime desc", 10, 3);
}
?>
<!DOCTYPE html>
<html>
