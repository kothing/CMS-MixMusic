...
if($ac == 'unucenter'){
	if(!$userlogined){
		...
		$db->query("update ".tname('user')." set in_ucid=0 where in_userid=".$missra_in_userid);
		echo 'return_2';
	}
}elseif($ac == 'unconnect'){
	if(!$userlogined){
	...
	if(!$userlogined){
		...
		$db->query("delete from ".tname('paylog')." where in_lock=1 and in_uid=".$missra_in_userid);
		$setarr = array(
		        'in_uid' => $missra_in_userid,
		        'in_uname' => $missra_in_username,
		        'in_title' => $title,
