...
switch($action){
	...
	case 'ucsubmit':
		...
		break;
	default:
		main(preg_match("/^(https?:\/\/)/i", UC_API) ? 1 : 0);
		break;
	}
?>
</body>
...
function ucsubmit(){
	...
	if(empty($s)) {
		...
		if(empty($ucs[0]) || empty($ucs[1])) {
			...
			$link = ucenter_db_connect($ucs[2], $ucs[4], $ucs[5], $ucs[3]);
			$connect = $link ? 'mysql' : '';
...
function ucenter_db_connect($dbhost, $dbuser, $dbpw, $dbname){
	if(extension_loaded('pdo_mysql')){
		...
		if(!($handle = @mysql_connect($dbhost, $dbuser, $dbpw, 1))){
			return false;
		}
