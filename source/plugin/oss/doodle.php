...
if($userid && isset($_GET['op']) && $_GET['op'] == 'doodle'){
	...
	if(!empty($matchs[1])){
        ...
        if($data){
	        $src = IN_ROOT.'./data/tmp/oss_'.date('YmdHis').rand(2,pow(2,24)).'_doodle.jpg';
	        $file = @fopen($src, 'wb');
	        @fwrite($file, $data);
	        @fclose($file);
	        $dst = $_SERVER['HTTP_HOST'].'/photo';
	        $object = $dst.'/'.$userid.'.'.date('YmdHis').rand(2,pow(2,24)).'.jpg';
