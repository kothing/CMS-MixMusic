...
if($ac == 'list'){
        ...
        while($row = $db->fetch_array($query)){
                ...
                while($rows = $db->fetch_array($result)){
                        $invisible = $db->getone("select in_invisible from ".tname('session')." where in_uid=".$rows['in_uids']);
                        $class = is_numeric($invisible) && $invisible == 0 ? 'online' : 'offline';
	...
	while($r = $db->fetch_array($res)){
		if(!$db->getone("select in_id from ".tname('friend')." where in_uid=".$missra_in_userid." and in_uids=".$r['in_userid'])){
                        $invisible = $db->getone("select in_invisible from ".tname('session')." where in_uid=".$r['in_userid']);
			$class = is_numeric($invisible) && $invisible == 0 ? 'online' : 'offline';
