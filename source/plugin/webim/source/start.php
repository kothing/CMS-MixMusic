...
if($ac == 'start'){
        ...
        if($type == 'num'){
                $invisible = $db->getone("select in_invisible from ".tname('session')." where in_uid=".$uid);
                $status = is_numeric($invisible) && $invisible == 0 ? 1 : 0;
                ...
                while($row = $db->fetch_array($query)){
                        $content = preg_replace('/\[emoji:(\d+)]/is', '<img src="'.IN_PATH.'static/pack/chat/emoji/emo_\1.gif">', $row['in_content']);
                        $content = preg_replace('/\[record:(\d+)]/is', '<embed src="'.IN_PATH.'static/pack/chat/player/record.swf?url='.IN_PATH.'data/tmp/\1.mp3&autoplay=0" wmode="opaque" width="261" height="23" type="application/x-shockwave-flash">', $content);
                        ...
                        $content = $row['in_id'] == $sid ? preg_replace('/\[event:shake]/is', '<script type="text/javascript">lib.shake(40, 16, "content", 56);</script><img src="'.IN_PATH.'static/pack/chat/icon/shake.png">', $content, 1) : preg_replace('/\[event:shake]/is', '<img src="'.IN_PATH.'static/pack/chat/icon/shake.png">', $content);
                        $content = preg_replace('/\[event:shake]/is', '<img src="'.IN_PATH.'static/pack/chat/icon/shake.png">', $content);
                        $content = preg_replace('/\[attach:(.*?):(.*?):(.*?)]/is', '<img src="'.IN_PATH.'static/pack/chat/icon/success.png" style="vertical-align:bottom">&nbsp;<a href="'.IN_PATH.'data/tmp/\1" target="_blank">\2[\3]</a>', $content);
